...
camera_relations = {
    'CAM_FRONT': ['CAM_FRONT_RIGHT', 'CAM_FRONT_LEFT', 'CAM_FRONT'],
    'CAM_FRONT_RIGHT': ['CAM_FRONT', 'CAM_BACK_RIGHT', 'CAM_FRONT_RIGHT'],
    'CAM_BACK_RIGHT': ['CAM_FRONT_RIGHT', 'CAM_BACK', 'CAM_BACK_RIGHT'],
    'CAM_BACK': ['CAM_BACK_RIGHT', 'CAM_BACK_LEFT', 'CAM_BACK'],
    'CAM_BACK_LEFT': ['CAM_BACK', 'CAM_FRONT_LEFT', 'CAM_BACK_LEFT'],
    'CAM_FRONT_LEFT': ['CAM_FRONT', 'CAM_BACK_LEFT', 'CAM_FRONT_LEFT'],
}


@cuda.jit
def project_pixels_cuda(output_image, output_weights, depth, original_image,
                        original_intrinsics, original_extrinsics, original_extrinsics_inv,
                        new_intrinsics, new_extrinsics, new_extrinsics_inv):
    ...
    if u_new < w and v_new < h:
        ...
        if point_original_camera[2] > 0:
            ...
            if 0 <= u_original < original_image.shape[1] and 0 <= v_original < original_image.shape[0]:
                ...
                # cuda.atomic.add(output_weights, (v_new, u_new), 1)


def run_project_pixels_cuda(cam_types, results, pre_images):
    r = []
    for _, new_cam_name in enumerate(cam_types):

        new_intrinsics, new_extrinsics = results["camera_intrinsics"][_], results["camera2ego"][_]
        ...
        for id, original_cam_name in enumerate(cam_types):
            if original_cam_name in camera_relations[original_cam_name]:
                original_cam_data = pre_images[id]

                try:
                    original_image_pil = original_cam_data
                    original_image = np.array(original_image_pil)
                    # Convert RGB to BGR to match OpenCV's format
                    original_image = original_image[:, :, ::-1].copy()
                except Exception as e:
                    print(f"Failed to load image: {original_cam_data}. Error: {e}")
                    continue

                original_intrinsics, original_extrinsics = results["real_camera_intrinsics"][id], results["real_camera2ego"][id]
                ...
                d_new_intrinsics = cuda.to_device(new_intrinsics)
                ...
                project_pixels_cuda[blockspergrid, threadsperblock](
                    d_output_image, d_output_weights, Dis, d_original_image,
                    d_original_intrinsics, d_original_extrinsics,
                    d_original_extrinsics_inv, d_new_intrinsics,
                    d_new_extrinsics, d_new_extrinsics_inv
                )
        ...
        # Normalize output image by the weights
        # output_image = output_image.astype(np.float32)
        # non_zero_weights = output_weights == 2
        # output_image[non_zero_weights] /= output_weights[non_zero_weights, None]
        # output_image[non_zero_weights] = (output_image[non_zero_weights] / output_weights[non_zero_weights, None])
        # output_image[non_zero_weights] = output_image[non_zero_weights] / 2

        output_image_rgb = output_image[:, :, ::-1]
        # output_image_rgb = np.clip(output_image, 0, 255).astype(np.uint8)[:, :, ::-1]
        output_image_pil = Image.fromarray(output_image_rgb)
        # output_image = output_image_pil.resize((1600,900), Image.BILINEAR)
        r.append(output_image_pil)
...
class LoadMultiViewImageFromFiles:
    ...
    def __call__(self, results):
        ...
        for _, name in enumerate(filename):
            # print(any_intrinsic[_], name)
            pre_images.append(Image.open(name))

        images = run_project_pixels_cuda(cam_types, results, pre_images)
